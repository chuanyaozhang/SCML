...
def parse_option():
    ...
    parser.add_argument('-its', '--iterations', type=int, help='number of episodes per epoch, default=100',  # tasks
                        default=100)  # tasks的数量，相当于有多少个batch

    parser.add_argument('-cTr', '--classes_per_it_tr',type=int,default=5,
    ...
    # model dataset
    parser.add_argument('--model', type=str, default='resnet12',help='resnet12,resnet18,resnet50')
        ...
        for it in range(self.iterations):
            ...
            for i in range(self.sample_suport_per_class+self.sample_query_per_class):
                for class_temp in class_sample:
                    ...
                    temp_class_index=[ i for i in class_index if i not in batch ]
                    one_class_index=random.sample(temp_class_index,k=1)#求列表差集
...
def main():
    ...
    log_name = './save/{}/{}/ktask{}_kway{}_ksuport{}_main_supcon'.format(opt.dataset,opt.method,opt.iterations,opt.classes_per_it_tr,opt.num_support_tr)
    # training routine
